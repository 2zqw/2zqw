...
class DESIGN():
    ...
    xrblue = f"\n{blueplus} 14D Bypass {BLUE}/ {WHITE}Instagram{BLUE}: {WHITE}@mzo9 {BLUE}/ {WHITE}@Y7CSS"
    xrblue2 = f"\n{blueplus} Auto Claimer {BLUE}/ {WHITE}Instagram{BLUE}: {WHITE}@mzo9 {BLUE}/ {WHITE}@Y7CSS"
class SETTINGS():
    def __init__(self):
...
class XNCE():
    def __init__(self, mode):
        ...
        if mode=="0":
            ...
            if self.logmode=="1":
                ...
                self.inex()
            self.current_user()
            self.new_username = "mzo9" + "".join(random.choices(string.ascii_lowercase+string.digits, k=10))
    ...
    def bypass_check1(self):
        ...
        if self.bypass:
            ...
            autopy.alert.alert(f"Done @{self.username}", "@mzo9")
            os.kill(os.getpid(), signal.SIGTERM)
        elif self.bypass==False and self.username_changed:
            self.run = False
            print(f"\r\n{DESIGN.redminus} Bypass = {DESIGN.RED}False")
            autopy.alert.alert(f"Failed @{self.username}", "@mzo9")
            os.kill(os.getpid(), signal.SIGTERM)
...
print(f"\n{DESIGN.blueplus} This Tool Is {DESIGN.YELLOW}By @Y7CSS ")
s = SETTINGS()
if s.settingsmode=="0": 
